...
if ($b1 gt $MapEdgeX) {#city
  ...
  # regular map, need city version here
  ...
  #      $tileset0=&loadvalidmap($map0);
  #      $tileset1=&loadvalidmap($map1);
  #      $tileset2=&loadvalidmap($map2);
  #      $tileset3=&loadvalidmap($map3);
  ...
  #
  # what do these do, used when walking on screens with no map?
  #  
  #if(!$tilestamp&&$player{'ts'}<0){
  #  $tilestamp=$cstamp;
  #  $tileset0=&randmap($map0);
  #  $tileset1=&randmap($map1);
  #  $tileset2=&randmap($map2);
  #  $tileset3=&randmap($map3);
  #}
  #if($tileset0){
  #  $player{'ts'}=$tilestamp;
  #  print "t0=$tileset0\n";
  ...
  $items=""; $static="";
  opendir(DIR,"$datadir/tokens/$map0/"); @data=readdir(DIR); closedir(DIR); &players(0);
  opendir(DIR,"$datadir/dynamic/$map0/"); @data=readdir(DIR); closedir(DIR); &items(0);
  opendir(DIR,"$datadir/static/$map0/"); @data=readdir(DIR); closedir(DIR); &static(0);
